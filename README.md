...
- Windows 10 / 11（64ビット）
- Python 3.13（64ビット、配布ZIPに公式インストーラーを同梱）
...
3. Python 3.13（64ビット）が未導入の場合は確認画面が表示されるので、`Y`を押します。
4. Pythonのインストール画面でインストールを実行します。既に入っている別バージョンのPythonは削除されません。
5. セットアップ後、デスクトップに作成される `Droste` ショートカットをダブルクリックします。黒いコマンド画面は表示されず、Windowsの通知領域にDrosteアイコンが常駐します。
6. PC管理画面 <http://localhost:5000/> が自動的に開きます。ブラウザーを閉じてもDrosteは動作を続けます。
7. 管理画面を再度開く場合は通知領域のDrosteアイコンをダブルクリックします。終了する場合は右クリックして「Drosteを終了」を選びます。
8. ショートカットを作成できない場合や起動エラーを確認する場合だけ、展開先の `regain.bat` を使用します。こちらは診断用のため黒いコマンド画面が表示され、閉じるとDrosteも終了します。

配布ZIPにはPython 3.13.14の公式64ビットインストーラーと `wheelhouse/` が含まれるため、受取側PCはインターネットを使わず準備できます。`setup.bat` はインストーラーのSHA-256とPython Software Foundationの電子署名を検証してから起動します。Python本体のライセンスは `PYTHON-LICENSE.txt` に同梱しています。
...
最終利用を想定するiPhone / iPadでは、証明書導入と端末登録が完了すると映像画面にホーム画面追加の案内が表示されます。

- iPhone / iPad: Safariの共有ボタンから「ホーム画面に追加」を選びます。
- Android: 動作確認用です。ホーム画面アプリとしての利用は対象外です。

以後は、iPhoneのホーム画面にあるDrosteアイコンを1回タップするだけで映像画面を開けます。ホーム画面から起動した後は、「ホーム画面に追加」の案内は表示されません。PC側では先にデスクトップの `Droste` ショートカットを起動しておく必要があります。
...
スクリプトは、Drosteが実際に待ち受けているIPとPython実行ファイルに対し、Windowsの「プライベート」ネットワークおよびローカルサブネットからの接続だけを許可します。Windowsのネットワークプロファイルが「パブリック」の場合は許可されません。
...
特に `tls/droste-ca-key.pem` を入手した人は、そのCAを信頼したスマートフォンに対する偽サーバー証明書を作成できるため、共有禁止です。新PCでは `setup.bat` と `regain.bat` を使って証明書を新規生成し、スマートフォンも新しく登録してください。
...
初回はPython公式サイトからPython 3.13.14の64ビットインストーラーと公式ライセンスを取得します。取得したインストーラーはSHA-256とPython Software Foundationの電子署名を検証し、以後は `vendor/` 内の検証済みファイルを再利用します。
