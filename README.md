...
5. セットアップ後、デスクトップに作成される `Droste` ショートカットをダブルクリックします。同じアイコンのショートカットからワンクリックで起動できます。
6. ショートカットを作成できない環境では、展開先の `regain.bat` をダブルクリックします。
7. PC管理画面 <http://localhost:5000/> が自動的に開きます。
...
以後は、iPhoneのホーム画面にあるDrosteアイコンを1回タップするだけで映像画面を開けます。ホーム画面から起動した後は、「ホーム画面に追加」の案内は表示されません。PC側では先にデスクトップの `Droste` ショートカット（または `regain.bat`）を起動しておく必要があります。
